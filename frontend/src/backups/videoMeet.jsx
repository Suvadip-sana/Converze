...
const videoMeetComponent = () => {
    ...
    let [username, setUsername] = useState("Anonymous") // If user join as not guest then it handel this
    ...
    const getPermissions = async () => {
        try {
            ...
            if (videoPermission) {
                ...
                window.localStream = videoPermission; // Set the local stream
                if (localVideoRef.current) {
                    localVideoRef.current.srcObject = videoPermission; // Set the video source
                }
            } else {
    ...
    let getUserMediaSuccess = (stream) => {
        try {
            if (window.localStream) {
                window.localStream.getTracks().forEach(track => track.stop()); // Clear previous tracks
            }
        } catch (error) {
            console.log(error);
        }
    
        window.localStream = stream; // Set the new stream
        localVideoRef.current.srcObject = stream; // Set the video source
    ...
    // TODO
    let addMessage = (data, sender, socketIdSender, userIdSender) => {

        const myUserId = localStorage.getItem("id");
        setMessages((prevMessages) => [
            ...prevMessages,
            { sender: sender, data: data, socketIdSender: socketIdSender, userIdSender: userIdSender }
        ]);

        if (userIdSender !== myUserId) {
    ...
    let connectToSocketServer = () => {
        const userId = localStorage.getItem("id"); // Get your persistent user ID
        if (!userId) {
        ...
        socketRef.current.on("connect", () => {
            socketRef.current.emit("join-call", window.location.href, userId); // Pass userId to the server
                    ...
                    for (let idto in connections) {
                        ...
                        connections[idto].createOffer().then((description) => {
                            connections[idto].setLocalDescription(description)
                                .then(() => {
                                    socketRef.current.emit("signal", idto, JSON.stringify({ "sdp": connections[idto].localDescription }));
                                })
                                .catch((err) => {
                                    console.log(err);
                                });
    ...
    let sendMessage = () => {
        ...
        socketRef.current.emit("chat-message", message, username, userId); // Send userId along with the message
        setMessage("");
    };
        ...
        <div>
            {askForUsername === true ?
                <div className={styles.lobby}>
                    ...
                    <TextField className={styles.lobbyTextfield} id="filled-basic" label="Username" value={username} onChange={e => setUsername(e.target.value)} size="small" variant="filled" />
                            ...
                            <div className={styles.chatContainer}>
                                ...
                                <div className={styles.chatDisplay} ref={chatDisplayRef}>
                                    ...
                                    {messages.length !== 0 ? messages.map((item, index) => {
                                        const myUserId = localStorage.getItem("id");
                                        const isSender = item.userIdSender === myUserId;
                                        return (
                                ...
                                <div className={styles.chattingTextfield}>
                                    ...
                                    />
                                    <Button onClick={sendMessage} disabled={message.trim() === ""} variant="contained" size="small">Send</Button>
                                </div>
                            </div>
                        </div>
