...
export const connectToSocket = (server) => {
    ...
    io.on("connection", (socket) => {  // socket: Represents the individual connection between the client and server.
        ...
        socket.on("join-call", (path, userId) => { // Listens for a client event named "join-call", triggered when a user joins a specific room (path). Path means room name,
            ...
            connections[path].push({socketId: socket.id, userId }); // If not then Push the socket id to the path array because there is more than one user connected with the same path.

            timeOnLine[socket.id] = new Date(); // To know when the user is connected with the server(Means when the user is online).

            for(let a = 0; a < connections[path].length; a++){
                io.to(connections[path][a].socketId).emit("user-joined", socket.id, connections[path]); // Emit the user-joined event to all the users who are connected with the same path if a new user is joind to the same room.
            }
            ...
            if(messages[path] !== undefined){
                for(let a = 0; a < messages[path].length; ++a){
                    io.to(socket.id).emit("chat-message", 
                        ...
                        messages[path][a]['userId'] // Add userId for persistent identification
                    );
                }
            }
        ...
        socket.on("chat-message", (data, sender, userId) =>{
            ...
            const matchingRoom = Object.keys(connections).find(room =>
                connections[room].some(conn => conn.socketId === socket.id)
            );
            ...
            if (matchingRoom) {
                ...
                console.log("Message", matchingRoom, ":", sender, data, "-----", userId);
        
                // Broadcast the message to all users in the room.
                connections[matchingRoom].forEach((element) => {
                    io.to(element.socketId).emit("chat-message", data, sender, socket.id, userId);
                });
            }
        });
        ...
        socket.on("disconnect", () => {
            ...
            for(const[room, users] of JSON.parse(JSON.stringify(Object.entries(connections)))){ // Iterate through all rooms to find where the disconnected user was. k means key, v means value
                const userIndx = users.findIndex( user => user.socketId === socket.id);
                if(userIndx !== -1){
                    key = room;
                    ...
                    // Remove the disconnected user's socket.id from the room's connection list.
                    connections[key].splice(userIndx, 1);
